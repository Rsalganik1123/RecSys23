...
def launch_redress(runs):
    ...
    if exp_name == 'BOOST': 
        ...
        # if not os.path.exists(f'{base_path}/{dataset_args.name}/{exp_name}/{run_args.method}/{run_args.version}/'): 
        #     os.mkdir(f'{base_path}/{dataset_args.name}/{exp_name}/{run_args.method}/{run_args.version}/')
    ...
    if u_epoch <  (cfg.TRAIN.UTILITY_EPOCHS -1)  or uf_epoch < (cfg.TRAIN.UTILITY_EPOCHS + cfg.TRAIN.FAIR_EPOCHS-1) : 
        # if u_epoch == -1: 
        #     fair_train_main(cfg) 
        # elif u_epoch <  (cfg.TRAIN.UTILITY_EPOCHS -1): 
        #     print(f"Starting from checkpoint:{checkpoint_path}")
        #     fair_train_main(cfg, pretrained_path = checkpoint_path) 
        # if uf_epoch < (cfg.TRAIN.UTILITY_EPOCHS + cfg.TRAIN.FAIR_EPOCHS-1): 
        #     print(f"Starting from checkpoint:{f_checkpoint_path}")
        #     fair_train_main(cfg, pretrained_path = f_checkpoint_path)
        fair_train_main(cfg) 
    ...
    # uf_rec_path = '{}{}'.format(validation_path, 'u_rec_tracks.pkl')
    # launch_performance_eval_clean(cfg, gen_amount=0, epoch = uf_epoch,recommended_track_path = uf_rec_path,  verbose=True, output_path=validation_path, mode = 'valid')
    # cfg.FAIR.POP_FEAT = 'appear_pop'
    # launch_fairness_audit_clean(cfg, k=100, epoch = uf_epoch, recommended_track_path=uf_rec_path, output_path=validation_path, mode='PS', setting = 'valid')
    # cfg.FAIR.POP_FEAT = '80_20_LT'
    # launch_fairness_audit_clean(cfg, k=100, epoch = uf_epoch,recommended_track_path=uf_rec_path, output_path=validation_path, mode='PS', setting = 'valid')
    # cfg.FAIR.POP_FEAT = 'log10_popcat'
    # launch_fairness_audit_clean(cfg, k=100, epoch = uf_epoch,recommended_track_path=uf_rec_path, output_path=validation_path, mode='PS', setting = 'valid')
    ...
    # #Loading Utility Recommendations -- TEST  
    # uf_val_rec_path = '{}{}'.format(fair_path, 'u_rec_tracks.pkl')
    # if len(glob.glob(uf_rec_path)) == 0: 
    #     uf_track_path = '{}{}'.format(fair_path, 'u_track_emb.pkl')
    #     uf_rec_df, uf_rec_path = gen_recommendations_cosine_clean(cfg, k=500, gen_amount=10, track_embed_path= uf_track_path , output_path=fair_path) 

    
    #Loading FairRecommendations -- TEST 
    # uf_rec_path = '{}{}'.format(fair_path, 'u_rec_tracks.pkl')
    # if len(glob.glob(uf_rec_path)) == 0: 
    #     uf_track_path = '{}{}'.format(fair_path, 'u_track_emb.pkl')
    #     uf_rec_df, uf_rec_path = gen_recommendations_cosine_clean(cfg, k=500, gen_amount=10, track_embed_path= uf_track_path , output_path=fair_path) 
...
def launch_scorereg(runs):
    ...
    # if dataset_args.name == 'LFM_Filtered': 
    #    data_path = '/home/mila/r/rebecca.salganik/scratch/MusicSAGE_data_final/LFM_Filtered/ScoreRegWithGenAmount'

    if dataset_args.name == 'MPD_Subset': 
        from src2.utils.config.full import get_cfg_defaults
        cfg = get_cfg_defaults()
        # data_path = '/home/mila/r/rebecca.salganik/scratch/MusicSAGE_Data/datasets/ScoreReg/10K_PID'
        data_path = f'{dataset_path}{dataset_args.name}/ScoreRegWithGenAmount2'
    ...
    # save_item_emb(model_path = model_path, output_path = emb_path, data_path = data_path, model_name = run_args.model)
    # rec_df, rec_path = gen_recommendations_cosine_clean(cfg, k=100, gen_amount=10, track_embed_path= emb_path , output_path=PS_path) 
    # rec_path = '{}{}'.format(PS_path, 'u_rec_tracks.pkl')
    # launch_performance_eval_clean(cfg, gen_amount=0, epoch = run_args.epochs, recommended_track_path = rec_path,  verbose=True, output_path=PS_path)
    # cfg.FAIR.POP_FEAT = '80_20_LT'
    # launch_fairness_audit_clean(cfg, k=100, epoch = run_args.epochs, recommended_track_path=rec_path, output_path=PS_path, mode='PS')
    # cfg.FAIR.POP_FEAT = 'log10_popcat'
    # launch_fairness_audit_clean(cfg, k=100, epoch = run_args.epochs, recommended_track_path=rec_path, output_path=PS_path, mode='PS')
    # cfg.FAIR.POP_FEAT = 'appear_pop'
    # launch_fairness_audit_clean(cfg, k=100, epoch = run_args.epochs, recommended_track_path=rec_path, output_path=PS_path, mode='PS')
...
def launch_xquad(runs):
    ...
    # launch_performance_eval_clean(cfg, gen_amount=0, epoch = "NONE",recommended_track_path = output_path,  verbose=True, output_path=exp_path)
    # launch_fairness_audit_clean(cfg, k=100, epoch = "NONE",recommended_track_path=output_path, output_path=exp_path, mode='PS')
    # launch_fairness_audit_clean(cfg, k=100, epoch="NONE", recommended_track_path=output_path, mode ='PS', LT_bin = 8, output_path=exp_path)
...
def launch_macr(runs):  
    ...
    # make_recs_OG(cfg, model_path=model_path, output_path=OG_path, data_path=data_path, model_name=exp_name, rec_num=100, c=30)
    # rec_path = '{}{}'.format(OG_path, 'rec_tracks.pkl')
    # launch_performance_eval_clean(cfg, gen_amount=0, epoch = run_args.epochs, recommended_track_path = rec_path,  verbose=True, output_path=OG_path)
    # cfg.FAIR.POP_FEAT = '80_20_LT'
    # launch_fairness_audit_clean(cfg, k=100, epoch = run_args.epochs, recommended_track_path=rec_path, output_path=OG_path, mode='PS')
    # cfg.FAIR.POP_FEAT = 'log10_popcat'
    # launch_fairness_audit_clean(cfg, k=100, epoch = run_args.epochs, recommended_track_path=rec_path, output_path=OG_path, mode='PS')
    # cfg.FAIR.POP_FEAT = 'appear_pop'
    # launch_fairness_audit_clean(cfg, k=100, epoch = run_args.epochs, recommended_track_path=rec_path, output_path=OG_path, mode='PS')
...
def launch_vanilla_pinsage(): 
    ...
    # if u_epoch <  (cfg.TRAIN.UTILITY_EPOCHS -1) : 
        # fair_train_main(cfg) 
...
def launch_vanilla_LGCN(runs): 
    ...
    # #REC GEN OG
    # make_recs_OG(cfg, model_path=model_path, output_path=OG_path, data_path=data_path, model_name=run_args.model, rec_num=100)
    # rec_path = '{}{}'.format(OG_path, 'rec_tracks_None.pkl')
    # launch_performance_eval_clean(cfg, gen_amount=0, epoch = run_args.epochs, recommended_track_path = rec_path,  verbose=True, output_path=OG_path)
    # cfg.FAIR.POP_FEAT = '80_20_LT'
    # launch_fairness_audit_clean(cfg, k=100, epoch = run_args.epochs, recommended_track_path=rec_path, output_path=OG_path, mode='PS')
    # cfg.FAIR.POP_FEAT = 'log10_popcat'
...
def launch_popularity_benchmark(runs): 
    ...
    # launch_performance_eval_clean(cfg, gen_amount=0, epoch = "NOPE", recommended_track_path = save_path,  verbose=True, output_path=exp_path)
    # launch_fairness_audit_clean(cfg, k=100, epoch = "NOPE",recommended_track_path=save_path, output_path=exp_path, mode='PS')
...
def launch_musfeat_benchmark(runs):
    ...
    # launch_performance_eval_clean(cfg, gen_amount=0, epoch = "NONE", recommended_track_path = rec_path,  verbose=True, output_path=exp_path)
    # launch_fairness_audit_clean(cfg, k=100, epoch = "NONE",recommended_track_path=rec_path, output_path=exp_path, mode='PS')    
...
def launch_vanilla_NGCF(runs): 
    scratch_path = '/home/mila/r/rebecca.salganik/scratch/PinSAGE_experiments/FULL_RUNS'  
    task_id = os.environ.get("SLURM_ARRAY_TASK_ID", 0)
    print(task_id, type(task_id))
    args_for_function = runs[int(task_id)] 
    print("RUNNING WITH ARGS:{}".format(args_for_function)) 
    exp_name, dataset_args, run_args = args_for_function.exp_name, args_for_function.dataset, args_for_function.run_params
    exp_path = '{}/{}/{}/{}_{}/'.format(scratch_path, dataset_args.name, exp_name, run_args.version, run_args.model)
    run_args = run_args._replace(output_path = exp_path)

    from src2.utils.config.LastFM import get_cfg_defaults
    cfg = get_cfg_defaults()
    data_path = '/home/mila/r/rebecca.salganik/scratch/MusicSAGE_data_final/LFM_Subset/Smaller_Size/Score_Reg'
    ...
    if gen_rec == 'PS': 
        save_item_emb(model_path = model_path, output_path = emb_path, data_path = data_path, model_name = run_args.model)
        rec_df, rec_path = gen_recommendations_cosine_clean(cfg, k=100, gen_amount=10, track_embed_path= emb_path , output_path=exp_path) 
    
    if gen_rec == 'OG': 
        make_recs_OG(cfg, model_path=model_path, output_path=exp_path, data_path=data_path, model_name=run_args.model, rec_num=100)

    rec_path = '{}{}'.format(exp_path, 'OG_rec_tracks.pkl')
    launch_performance_eval_clean(cfg, gen_amount=0, epoch = run_args.epochs, recommended_track_path = rec_path,  verbose=True, output_path=exp_path)
    launch_fairness_audit_clean(cfg, k=100, epoch = run_args.epochs, recommended_track_path=rec_path, output_path=exp_path, mode='PS')
...
def launch_IPW(): 
    ...
    print("RUNNING WITH ARGS:{}".format(args_for_function)) 
    exp_name, dataset_args, run_args = args_for_function.exp_name, args_for_function.dataset, args_for_function.run_params
    exp_path = '{}/{}/{}/{}/'.format(scratch_path, dataset_args.name, 'IPW', run_args.version)
    ...
    if len(glob.glob(u_rec_path)) == 0: 
        u_track_path = '{}{}'.format(utility_path, 'u_track_emb.pkl')
        u_rec_df, u_rec_path = gen_recommendations_cosine_clean(cfg, k=100, gen_amount=10, track_embed_path= u_track_path , output_path=utility_path) 
        
    #Evaluate 
    u_rec_path = '{}{}'.format(utility_path, 'u_rec_tracks.pkl')
    launch_performance_eval_clean(cfg, gen_amount=0, epoch = u_epoch,recommended_track_path = u_rec_path,  verbose=True, output_path=utility_path)
    launch_fairness_audit_clean(cfg, k=100, epoch = u_epoch,recommended_track_path=u_rec_path, output_path=utility_path, mode='PS')
    # launch_fairness_audit_clean(cfg, k=100, epoch = u_epoch,recommended_track_path=u_rec_path, output_path=utility_path, mode='PS', LT_bin = 0)
...
def test(): 
    ...
    task_id = os.environ.get("SLURM_ARRAY_TASK_ID", 0)
    print(task_id, type(task_id))
    args_for_function = REDRESS_runs_MPD[int(task_id)] 
    print("RUNNING WITH ARGS:{}".format(args_for_function)) 
    exp_name, dataset_args, run_args = args_for_function.exp_name, args_for_function.dataset, args_for_function.run_params
    exp_path = '/home/mila/r/rebecca.salganik/scratch/PinSAGE_experiments/FULL_RUNS/MPD_Subset_OLD/REDRESS/v1_G_0.1_A_0.01_B_0.0'
    #'{}/{}/{}/{}_G_{}_A_{}_B_{}'.format(scratch_path, dataset_args.name, exp_name, run_args.version, run_args.gamma, run_args.alpha, run_args.boost)
    ...
    checkpoint_path, u_epoch = find_latest_checkpoint_clean(exp_path, mode='u')
    # track_embeddings, u_track_path = gen_track_embeddings_clean(cfg, checkpoint_path, output_path = None)

    # u_track_path = '/home/mila/r/rebecca.salganik/scratch/PinSAGE_experiments/FULL_RUNS/LFM_Filtered/REDRESS/v1_G_0.1_A_0.01_B_0.0/utility/u_track_emb.pkl'
    # u_rec_df, u_rec_path = gen_recommendations_cosine_clean(cfg, k=500, gen_amount=10, track_embed_path= u_track_path , output_path=None) 
        
    validation_tests(cfg, checkpoint_path)
         
def test_from_checkpoint(): 
    best_checkpoint = '/home/mila/r/rebecca.salganik/scratch/PinSAGE_experiments/FULL_RUNS/LFM_Subset/REDRESS/v1_G_0.3_A_0.01_B_0.0/best_epoch/u_model_bak_000002.pt'

def tiny_runs_redress(runs): 
    task_id = os.environ.get("SLURM_ARRAY_TASK_ID", 0)
    jobid = os.environ.get("SLURM_ARRAY_JOB_ID", 0)
    print(task_id, type(task_id))
    args_for_function = runs[int(task_id)] 
    print("RUNNING WITH ARGS:{}".format(args_for_function)) 
    base_path, exp_name, _, run_args = args_for_function.base_path, args_for_function.exp_name, args_for_function.dataset, args_for_function.run_params

    exp_path = '{}/TINY_CF/G_{}_A_{}/'.format(base_path, run_args.gamma, run_args.alpha)
    
    from src2.utils.config.full import get_cfg_defaults
    cfg = get_cfg_defaults()

    tiny_cf = '/home/mila/r/rebecca.salganik/scratch/MusicSAGE_Data_Final2/TINY_CF/'
    cfg.DATASET.DATA_PATH = tiny_cf + 'train_val4.pkl'
    cfg.DATASET.TEST_DATA_PATH = tiny_cf + 'test.pkl'
    cfg.DATASET.NAME = 'NO_ISOLATE' 
    cfg.TRAIN.SOLVER.BASE_LR = 0.0001
    cfg.TRAIN.SOLVER.DECAY = False 
    cfg.TRAIN.LOSS = 'FOCAL_LOSS'
    ...
    cfg.TRAIN.UTILITY_EPOCHS =15
    cfg.TRAIN.FAIR_EPOCHS = 15
    cfg.TRAIN.BATCHES_PER_FAIRNESS_EPOCH = 500
    cfg.TRAIN.BATCHES_PER_UTILITY_EPOCH = 1000
...
def tiny_runs_MACR(runs): 
    ...
    base_path, exp_name, dataset_args, run_args = args_for_function.base_path,args_for_function.exp_name, args_for_function.dataset, args_for_function.run_params
    # exp_path = '{}/{}/{}/{}_{}_MACR/'.format(base_path, dataset_args.name, exp_name, run_args.version, run_args.model, run_args.weight)
    exp_path = '{}/TINY_CF/MACR/'.format(base_path, run_args.weight)
    run_args = run_args._replace(output_path = exp_path)
    ...
    data_path = '/home/mila/r/rebecca.salganik/scratch/MusicSAGE_Data_Final2/TINY_CF/ScoreRegWithGenAmount2'
    ...
    track_emb_path = exp_path + 'item_emb.pkl'
    
    

    #REC GEN OG
    for c in [5]:
        make_recs_OG(cfg, model_path=model_path, output_path=exp_path, data_path=data_path, model_name='MACR', rec_num=100, c=c, mode='test', track_path = track_emb_path)
...
if __name__ == "__main__": 
    ...
    if args.mode == 'EXP_DROPOOUT': 
        launch_redress(dropout_runs_LFM)
        # launch_redress(BOOST_hp_runs_MPD) 
    ...
    if args.mode == 'LGCN_LR': 
        if args.dataset == 'MPD': 
            print("NOPE")
        if args.dataset == 'LFM': 
            launch_vanilla_LGCN(lr_hp_runs_LFM)
    
    if args.mode == 'LGCN_D2_TEST': 
        if args.dataset == 'MPD': 
            launch_vanilla_LGCN(d2_test)
    
    if args.mode == 'TINY_CF': 
        tiny_runs_redress(tiny_redress_run)
        # tiny_runs_MACR(tiny_redress_run)
