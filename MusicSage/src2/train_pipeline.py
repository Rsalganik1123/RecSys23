...
def train_epoch(cfg, model, dataloader_train, dataloader_val, optimizer, tb_writer, global_steps, scaler, epoch, train_g, valid=True):
    ...
    # print('TRAIN: batch, loss, auc')
    # for batch_id in tqdm(range(cfg.TRAIN.BATCHES_PER_EPOCH)): #cfg.TRAIN.BATCHES_PER_EPOCH
        # if not cfg.FP16:
    ...
    for batch_id in tqdm(range(cfg.TRAIN.BATCHES_PER_EPOCH)):
        ...
        pos_graph = pos_graph.to(device)
        neg_graph = neg_graph.to(device)
        # print(blocks[-1].dstdata['feats'])
        #Train
        pos_score, neg_score, loss, auc, ap, h_item = model(pos_graph, neg_graph, blocks)
        # print(blocks[-1].dstdata['feats'])
        # exit() 
        #Save emb 
        # train_g.nodes['track'].data['emb'][pos_graph.ndata[dgl.NID]] = h_item.cpu()
        #Back Prop 
        optimizer.zero_grad()
        loss.backward()
    ...
    if valid:
        ...
        with torch.no_grad():
            for batch_id in range(100): #cfg.TRAIN.BATCHES_PER_EPOCH
...
def train(cfg):
    ...
    # emb = model.get_repr(g)
    # print(emb.shape)

    # train_g.nodes['track'].data['emb'] = torch.zeros((train_g.num_nodes('track'), cfg.MODEL.PINSAGE.HIDDEN_SIZE))
    ...
    #playlist dataloader 
    # playlist_sampler = PlaylistBatchSampler(train_g, 2)
    # edge_scorer = UsertoItemScorer_alone(train_g , cfg)
    ...
    for epoch_idx in range(all_epochs):
        ...
        # if playlist_sampler: 
        #     playlist_nodes = next(iter(playlist_sampler))
        #     edge_scores = edge_scorer(playlist_nodes)
        #     print(edge_scores)
        #     exit() 
...
from src2.sampler.node_sampler.playlist_sampler import PlaylistBatchSampler 
from src2.model.loss_fn import * 
def UI_eval(cfg, emb_path): 
    #load graph 
    # ipdb.set_trace() 
    g, train_g, val_g = build_dataset(cfg)
    print("Loaded graphs: Full:{}, \nTrain:{}, \nValidation:{}".format(g, train_g, val_g))
    ...
    #load scorer 
    edge_scorer = UsertoItemScorer()
    playlist_embs = edge_scorer.get_playlist_reps(g, h)
    print("loaded playlist embeddings", playlist_embs.shape)

    #initialize playlist dataloader 
    g.nodes['track'].data['h'] = h
    g.nodes['playlist'].data['h'] = playlist_embs
    playlist_set = PlaylistBatchSampler(g)
    cross_entropy = [] 
    for i in tqdm(range(len(playlist_set))): 
        p, pos, neg = playlist_set[i]
        # print(p, pos, neg)
        pos_scores = edge_scorer(pos).reshape(-1, 1)
        neg_scores = edge_scorer(neg).reshape(-1, 1)
        cross_entropy.append(xentropy(pos_scores, neg_scores).item()) 
    print("AVERAGE CROSS ENTROPY", np.mean(cross_entropy)) 
