...
#     nodelist = track_idx.copy() #list(range(n)) #ist(range(1, n)) #track_idx.copy()  #list(range(1, n)) 
...
#     p = 0
#     while p < len(nodelist): #changed this --> nodelist was just track_idx
#         u = nodelist[p]
#         print(u, height)
#         old_height = height[u]
#         discharge(u)
...
# def calculate_weights(rec_df, lmbda, num_tracks, num_plists, global_rel, verbose=False):
#     nodes = num_plists + num_tracks + 2
#     W = np.zeros((nodes, nodes))
#     for u_idx in range(len(rec_df.pid.unique())): 
#         pid = rec_df.pid.unique()[u_idx]
#         row = rec_df[rec_df.pid == pid]
#         tracks = row.recs.to_list() #[0]
#         for i_idx in range(len(tracks)): 
#             track = tracks[i_idx]
#             try: 
#                 weight = lmbda * i_idx + (1-lmbda) * global_rel[track]
#                 u = u_idx + 1
#                 i = num_plists + track
#                 W[u][i] = W[i][u] = weight
#                 if verbose: 
#                     print("p:{}, t:{}, rank:{}, degree:{}, weight:{} " .format(u, i, i_idx, global_rel[track], weight)) 
#             except: continue 
#     return W, np.sum(W)
...
# def build_graph(exploded, lmbda): 
...
#     plist_idx = list(range(1, num_plists+1))
#     track_idx = list(range(plist_idx[-1]+1, plist_idx[-1]+1+num_tracks+1)) 
#     W_raw,cap = calculate_weights(exploded, lmbda, num_tracks, num_plists, global_rel)
...
# def generate_candidates(rec_df, lmbda=0.2, verbose=True): 
#     candidates = [] 
#     counter = 0  
#     exploded = rec_df.explode('recs').reset_index() 
#     run = True 
#     G, plist_idx, track_idx, num_tracks, num_plists =  build_graph(exploded, lmbda)
#     # ipdb.set_trace() 
#     while True: 
#         # print(G)
...
# # u_c = [(1, 0), (2, 0), (0, 4), (1, 4), (2, 4), (2, 5), (0, 8), (2, 8), (0, 9), (1, 9), (2, 9)]
# # # exploded[((exploded['pid']) & (exploded['recs'] == tid))].index
# # exploded,num_plists, num_tracks = update_G(u_c, exploded)
# # print(exploded, num_plists, num_tracks)



# generate_candidates(rec_df)
...
def MaxFlow_sp(C, U, I, track_idx, plist_idx):
    ...
    #F = np.zeros((n,n)) #convert to int  --> needs to change
...
def test_main(): 
    ...
    candidates = generate_candidates(rec_df)
    final_df = make_rec_list(rec_df, candidates, beta=0.2, n=5)
    print(final_df)
...
# p  = '/home/mila/r/rebecca.salganik/scratch/PinSAGE_experiments/ablations_small_100_10/fairness_hp/ALPHA/v1__G_0.7_A_0.01_B_0.0/u_recs_TS1_top500/cosine_10_recommended_tracks.pkl'

# push_relabel_main(p, map_idx=True, beta=0.2, n=100, output_path=None, sparse=False) 
