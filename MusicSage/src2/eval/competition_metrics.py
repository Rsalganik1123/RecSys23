...
# def compute_ndcg(preds, gt_tids):
#     preds = preds[:500]
#     relevances = np.array([x in preds for x in gt_tids])
#     # relevances = np.array([x in preds for x in gt_tids])
#     overlap = np.sum(relevances)
#     log_bottom = np.log2(np.arange(2, len(preds)+1))
#     if overlap > 0:
#         dcg = relevances[0] + np.sum(relevances[1:] / log_bottom[:len(gt_tids) - 1])
...
# def compute_ndcg(preds, known_num, gt_tids):
#     preds = __get_unique(preds) 
#     gt_tids = __get_unique(gt_tids)
#     #DCG 
#     score_dcg = [float(el in gt_tids) for el in preds]
#     dcg = np.sum(score_dcg / np.log2(1 + np.arange(1, len(score_dcg) + 1)))
#     #IDCG 
#     score_idcg = [float(el in gt_tids) for el in gt_tids]
#     idcg = np.sum(score_idcg / np.log2(1 + np.arange(1, len(score_idcg) + 1)))
#     ndcg = dcg / idcg
#     return ndcg
...
def competition_eval(cfg, recommended_tracks, gen_amount, output_path, verbose=False, mode = 'test'): # (preds, known_num, all_tids, tracks): 
    ...
    for p in tqdm(test_set.pid.unique()):
        ...
        # prec_at_500.append(precisionk(recs, gen_amount, associated_tracks, k=500))
        # prec_at_1K.append(precisionk(recs, gen_amount, associated_tracks, k=1000))
        # prec_at_10K.append(precisionk(recs, gen_amount, associated_tracks, k=10000))
        # recall_at_500.append(precisionk(recs, gen_amount, associated_tracks, k=500))
        # recall_at_1K.append(precisionk(recs, gen_amount, associated_tracks, k=1000))
        # recall_at_10K.append(precisionk(recs, gen_amount, associated_tracks, k=10000))
        # normal_ndcg_at_500.append(normal_ndcgk(recs, associated_tracks, 500))
        # normal_ndcg_at_1K.append(normal_ndcgk(recs, associated_tracks, 1000))
        # normal_ndcg_at_10K.append(normal_ndcgk(recs, associated_tracks, 10000))
        # ndcg_all.append(compute_ndcg_all(recs, gen_amount, associated_tracks))
        # prec_at_k.append(r_precision_old(recs, gen_amount, associated_tracks))
        # strict_prec.append(strict_r_precision(recs, gen_amount, associated_tracks))
        # artist_prec.append(r_precision_artist(recs, gen_amount, associated_tracks, df_items))
        # ballpark_prec.append(r_precision(recs, gen_amount, associated_tracks))
        # ballpark_ndcg.append(compute_ndcg_all(recs, gen_amount, associated_tracks))
        
    if verbose and output_path: 
        # metrics_df = pd.DataFrame({'pid':p, 'artist_prec': artist_prec, 'ballpark_prec': ballpark_prec, 'strict_prec': strict_prec, 'strict_ndcg': strict_ndcg})
        ...
        #'precision@500': prec_at_500, 'precision@1K': prec_at_1K, 'precision@10K': prec_at_10K,
        #'recall@500': recall_at_500, 'recall@1K': recall_at_1K, 'recall@10K': recall_at_10K,
        #'normal_ndcg@500': normal_ndcg_at_500, 'normal_ndcg@1K': normal_ndcg_at_1K,'normal_ndcg@10K': normal_ndcg_at_10K
    ...
    performance_metrics = {
        ...
        # 'precision@500': np.mean(prec_at_500),
        # 'precision@1K': np.mean(prec_at_1K), 
        # 'precision@10K': np.mean(prec_at_10K), 
        # 'recall@500': np.mean(recall_at_500), 
        # 'recall@1K': np.mean(recall_at_1K), 
        # 'recall@10K': np.mean(recall_at_10K),
        # 'normal_ndcg@500': np.mean(normal_ndcg_at_500), 
        # 'normal_ndcg@1K': np.mean(normal_ndcg_at_1K),
        # 'normal_ndcg@10K': np.mean(normal_ndcg_at_10K), 
        # 'ndcg_all': np.mean(ndcg_all)
        # 'artist_prec': np.mean(artist_prec) , 
        # 'ballpark_prec': np.mean(ballpark_prec), 
        # # 'strict_ballpark_prec': np.mean(strict_ballpark_prec), 
        # 'strict_prec': np.mean(strict_prec), 
        # # 'ballpark_ndcg': np.mean(ballpark_ndcg), 
        # 'strict_ndcg': np.mean(strict_ndcg), 
    }
