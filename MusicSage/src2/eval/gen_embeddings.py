...
def gen_track_embeddings(cfg,  checkpoint_path, mode = 'fullg', output_path=None): 
    ...
    for blocks in tqdm(dataloader_it):
        ...
        with torch.no_grad():
            ...
            # all_ids.append(blocks[1].dstnodes['track'].data['id'])
    all_features_array = np.concatenate(all_features_cpu)
    # all_features_df = pd.DataFrame({'tid':g.nodes['track'].data['id'], 'emb': all_features_cpu})
    #SAVE
    if output_path:
        if not os.path.exists(output_path): 
            os.mkdir(output_path)
        ...
        pickle.dump(all_features_array, open(file_path, "wb"))
            ...
            # pickle.dump(all_features_array, open(os.path.join(output_path,'embeddings_as_array.pkl'), "wb"))
        # pickle.dump(all_features_df, open(output_path +'embeddings_as_df.pkl', "wb"))
