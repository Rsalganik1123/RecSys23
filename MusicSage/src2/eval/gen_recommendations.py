...
def sim(playlist_embedding, track_embeddings, k, version):
    ...
    if version == 'dot_prod': 
        # r = [] 
        # for t in track_embeddings: 
        #     r.append(torch.dot(playlist_embedding, t.T)) 
        # return torch.tensor(r)  
        # r = torch.Tensor([playlist_embedding @ t for t in track_embeddings])
        # print(playlist_embedding.shape, track_embeddings.shape)
    ...
    # if version == 4: 
    #     track_norm = norm(track_embeddings, axis=1)
    #     playlist_norm = norm(playlist_embedding)
    #     print(track_embeddings.shape, playlist_embedding.shape, track_norm.shape, playlist_norm.shape)
    #     r = (track_embeddings @ playlist_embedding.T)/(track_norm * playlist_norm)
    #     recs = torch.topk(r, k)[1].tolist() 
...
def gen_recommendations_overlap(cfg, k, gen_amount, mode,track_embeddings=None, track_embed_path=None, output_path=None): 
    ...
    for pid in tqdm(test_set.pid.unique()): 
        ...
        playlist_embeddings.append(playlist_embedding)
        recs = sim(playlist_embedding, track_embeddings, k, 3)
        recs = sim(playlist_embedding, track_embeddings, k, 4)
        # r = torch.Tensor(cosine_similarity(playlist_embedding, track_embeddings))
        recs = torch.topk(r, k)[1].tolist() 
        # rec_tracks = df_items[df_items.tid.isin(recs)].tid.tolist() 
        recommended_tracks.append(recs) #rec_tracks or recs? 
    rec_df = pd.DataFrame({'pid': test_set.pid.unique(), 'recs': recommended_tracks})

    if output_path: 
        if not os.path.exists(output_path): 
            os.mkdir(output_path)
        file_path = os.path.join(output_path,"{}_recommended_tracks.pkl".format(mode))
        print("***Saving Recommended Track List to {}***".format(file_path))
        pickle.dump(rec_df, open(file_path, "wb"))
        playlist_file_path = os.path.join(output_path,"mean{}_playlists.pkl".format(gen_amount))
        pickle.dump(playlist_embeddings, open(playlist_file_path, "wb"))
    return rec_df
...
def gen_recommendations_cosine(cfg, k, gen_amount, sim_version='cosine', track_embeddings=None, track_embed_path=None, output_path=None): 
    ...
    for pid in tqdm(test_set.pid.unique()): 
        ...
        rec_tracks = df_items[df_items.tid.isin(recs)].tid.tolist() 
        # print(recs, rec_tracks)
        # assert len([i for i in recs if i not in rec_tracks]) == 0 
